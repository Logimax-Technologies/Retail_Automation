...
class ApprovalRateFixing(unittest.TestCase):
    """
    Approval Rate Fixing Automation
    ...
    def test_approval_rate_fixing(self):
        """Main entry point for Approval Rate Fixing automation"""
        driver = self.driver
        wait = self.wait

        # Navigate to Approval Rate Fixing List
        try:
            if "approval_rate_fixing" not in driver.current_url:
                ...
                # Need to click Rate Fixing first to expand, then Approval Rate Fixing
                Function_Call.click(self, "//span[contains(text(), 'Rate Fixing')]")
                sleep(1)
                Function_Call.click(self, "//a[contains(@href, 'approval_rate_fixing')]")
                sleep(2)
                print("✅ Navigated to Approval Rate Fixing list page")
        ...
        for row_num in range(2, valid_rows):
            ...
            try:
                if "CANCEL" in tc_id:
                    ...
                    print("🔄 Executing Scenario: Standard Approval Rate Fix Entry")
                ...
                if result[0] == "Pass" and "CANCEL" not in tc_id and "APPROVE" not in tc_id:
                    print(f"🔍 Verifying Approval Rate Fix {ratebox_id} in List Page...")
        ...
        workbook.close()
        print(f"\n{'='*80}")
        print(f"✅ Approval Rate Fixing Automation Completed")
        print(f"{'='*80}")
    ...
    def test_standard_rate_fix(self, row_data, row_num, sheet_name):
        ...
        try:
            ...
            # Click Add button / Navigate directly
            current_field = "Add Button"
            driver.get(ExcelUtils.BASE_URL + "index.php/admin_ret_purchase/rate_fixing/approval_rate_fixing_add")
            sleep(3)
            ...
            if row_data["Karigar"]:
                ...
                sleep(2)  # Wait for Approval Ratefix No dropdown to load via AJAX
            ...
            try:
                ...
                msg = "Item details table did not load after selecting Approval Ratefix No ⚠️"
            ...
            if row_count > 1:
                try:
                    child_sheet_name = "ApprovalRateFixingItems"
                    ...
                    for cr in range(2, child_valid_rows):
                        ...
                        if str(c_tc_id) == str(row_data["TestCaseId"]):
                            child_rows_data.append({
                                "FixWt": child_sheet.cell(row=cr, column=4).value,
                                "RateExclTax": child_sheet.cell(row=cr, column=5).value,
                                "GSTPercent": child_sheet.cell(row=cr, column=6).value
                            })
                    wb.close()
                except Exception as e:
            ...
            for i in range(1, row_count + 1):
                ...
                if len(child_rows_data) > 0 and len(child_rows_data) >= i:
                    item_data = child_rows_data[i-1]
                
                fix_wt = str(item_data.get("FixWt") or 0)
                rate_val = str(item_data.get("RateExclTax") or 0)

                # Fix Wt
                fix_input = driver.find_element(By.XPATH, f"(//table[@id='item_details']/tbody/tr)[{i}]//input[contains(@name, 'fix_weight')]")
                fix_input.clear()
                fix_input.send_keys(fix_wt)
                
                # Rate Excl Tax
                rate_input = driver.find_element(By.XPATH, f"(//table[@id='item_details']/tbody/tr)[{i}]//input[contains(@name, 'rate_per_gram')]")
                rate_input.clear()
                ...
                try:
                    ...
                    print(f"Row {i} Filled but failed to read calculated UI rows: {e}")
    ...
    def test_list_verification(self, ratefix_id, row_data):
        ...
        try:
            ...
            # ── Search Rate Fix ID / Approval Ratefix No in DataTable ──
            search_term = str(row_data.get("ApprovalRatefixNo") or ratefix_id)
    ...
    def test_cancel_flow(self, row_data, row_num, sheet_name):
        ...
        try:
            ...
            print("📝 Creating Approval Rate Fix to test cancellation...")
    ...
    def test_approve_flow(self, row_data, row_num, sheet_name):
        ...
        try:
            # First create the entry
            print("📝 Creating Approval Rate Fix to test Approval...")
            create_result = self.test_standard_rate_fix(row_data, row_num, sheet_name)
            if create_result[0] != "Pass":
                return ("Fail", f"❌ Record creation failed: {create_result[1]}")
    ...
    def _capture_save_result(self, row_data, success_keyword, explicit_id=None):
        try:
            ...
            elif "success" in alert.lower() or "successful" in alert.lower() or "added" in alert.lower():
